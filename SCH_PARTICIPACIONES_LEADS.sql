 TRUNCATE TABLE `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Stg_P2C_Part_Leads`;

insert into  `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Stg_P2C_Part_Leads`
...
, C.IP
,C.CookieID
,cast(FORMAT_TIMESTAMP('%Y-%m-%d', PARSE_TIMESTAMP('%Y-%m-%d', LEFT(C.Timestamp,10)))as DATE) AS TIMESTAMP
,FORMAT_TIMESTAMP('%H %M %S', cast(FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', PARSE_TIMESTAMP('%Y-%m-%d %H:%M:%S', LEFT(C.Timestamp,19)))as TIMESTAMP))  AS HORA
,1 AS IS_CONVERSION
,0 AS NUMERO_ORDEN
,0 P2C_ID
,C.CAMPAIGN
...
,  -- HOMOLOGA CHANNEL
    ...
    ,C.MEDIA AS MEDIO
    ,cast(FORMAT_TIMESTAMP('%Y-%m-%d', PARSE_TIMESTAMP('%Y-%m-%d', LEFT(C.Timestamp,10)))as DATE) AS FECHA_CONVERSION
 FROM  `primal-sunup-357522.TEC_EXCELLENCE_ADFORM_CONVERSION.Transform_Adform_Leads_Weddings` C
 where 
 -- C.IS_BOOKING_CENTER IN ('false', 'NA')
 -- AND C.CookieID <> '0'
 -- C.SALES is not null 
   EXTRACT(YEAR FROM C.FECHA) = EXTRACT(YEAR FROM CURRENT_DATE()) -- VAR AÑO ACTUAL
 and EXTRACT(MONTH FROM C.FECHA) = EXTRACT(MONTH FROM CURRENT_DATE()) -- VAR MES ANTERIOR
and EXTRACT(DAY FROM C.FECHA) BETWEEN 01 AND EXTRACT(DAY FROM DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY))
...
END AS CAMPAIGN_FUNNEL
 ...
 FROM  `TEC_EXCELLENCE_ADFORM_CLICS_IMPRESSION.Transform_Adform_Clics` CL
  INNER JOIN (SELECT IP,COOKIEID,MAX(TIMESTAMP)AS TIMESTAMP,  FROM `primal-sunup-357522.TEC_EXCELLENCE_ADFORM_CONVERSION.Transform_Adform_Leads_Weddings`
 WHERE 
  EXTRACT(YEAR FROM CAST(TIMESTAMP AS TIMESTAMP))  = EXTRACT(YEAR FROM CURRENT_DATE())  -- VAR AÑO ACTUAL
and EXTRACT(MONTH FROM CAST(TIMESTAMP AS TIMESTAMP))  = EXTRACT(MONTH FROM CURRENT_DATE()) and 
EXTRACT(DAY FROM CAST(TIMESTAMP AS TIMESTAMP)) BETWEEN 01 AND EXTRACT(DAY FROM DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)) GROUP BY 1,2 ) C
ON C.IP = CL.IP AND C.CookieID = CL.CookieID 
 WHERE CL.CookieID <> '0'
  and CAST(CL.Timestamp AS TIMESTAMP) BETWEEN DATE_SUB(CAST(C.TIMESTAMP AS TIMESTAMP), INTERVAL 60 DAY) AND CAST(C.TIMESTAMP AS TIMESTAMP)
...
END AS CAMPAIGN_FUNNEL
 ...
 INNER JOIN (SELECT IP,COOKIEID,MAX(TIMESTAMP)AS TIMESTAMP,  FROM `primal-sunup-357522.TEC_EXCELLENCE_ADFORM_CONVERSION.Transform_Adform_Leads_Weddings`
 WHERE EXTRACT(YEAR FROM CAST(TIMESTAMP AS TIMESTAMP)) = EXTRACT(YEAR FROM CURRENT_DATE()) -- VAR AÑO ACTUAL
and EXTRACT(MONTH FROM CAST(TIMESTAMP AS TIMESTAMP)) = EXTRACT(MONTH FROM CURRENT_DATE()) 
and EXTRACT(DAY FROM CAST(TIMESTAMP AS TIMESTAMP)) BETWEEN 01 AND EXTRACT(DAY FROM DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY))
 GROUP BY 1,2 ) C
ON C.IP = IM.IP AND C.CookieID = IM.CookieID 
 WHERE IM.CookieID <> '0'
  and CAST(IM.Timestamp AS TIMESTAMP) BETWEEN DATE_SUB(CAST(C.TIMESTAMP AS TIMESTAMP), INTERVAL 60 DAY) AND CAST(C.TIMESTAMP AS TIMESTAMP)
...
DELETE FROM `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` WHERE ORIGEN = 'PARTICIPACIONES LEADS'
...
0 AS REVENUE_RES,
0 AS  LEADS_RES,
'PARTICIPACIONES LEADS' AS ORIGEN,
0 AS PP_LEADS_OBJ,
Paticipaciones AS PP_LEADS_RES
FROM `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Participaciones_Leads` WHERE FUNNEL <> 'NA';

update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set canal = 'Mixto' where CANAL = 'MIXTO' AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set canal = 'Display' where CANAL = 'DISPLAY' AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set canal = 'Video' where CANAL = 'VIDEO' AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set MEDIO = 'META' where MEDIO IN ('FACEBOOK','Facebook') AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set canal = 'Search' where CANAL = 'SEARCH' AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set canal = 'Search' where REGEXP_CONTAINS(CANAL, r'(?i)(MICROSOFT|GOOGLE)') AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set canal = 'Display' where REGEXP_CONTAINS(CANAL, r'(?i)(RTB)') AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set FUNNEL = 'TOFU'
WHERE FUNNEL = 'MOFU' AND CANAL = 'Display' and CAMPANIA = 'WEDDINGS' AND MEDIO = 'Bride Click' AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set FUNNEL = 'TOFU'
WHERE FUNNEL = 'MOFU' AND CANAL = 'Display' and CAMPANIA = 'WEDDINGS' AND MEDIO = 'PINTEREST' AND ORIGEN = 'PARTICIPACIONES LEADS';
update `primal-sunup-357522.TEC_LAYOUT_OBJETIVOS.TEC_Objetivos_dia` set FUNNEL = 'BOFU' where MEDIO IN ('MICROSOFTPMAX') AND
FECHA >= '2025-02-01' AND ORIGEN = 'PARTICIPACIONES LEADS';
